...
<?php
$cat = get_the_category()[0]->name;
$source = (ICL_LANGUAGE_CODE == 'nl') ? 'bron' : 'source';
$mediaText = (ICL_LANGUAGE_CODE == 'nl') ? 1939 : 3295;
if ($cat == 'Actueel' || $cat == 'Current') {
  echo '<div class="actueelPostInfo">';
  echo '<div class="actueelBron">';
  echo 'Bron:</br>';
  echo get_field($source, get_the_ID());
  echo ' </div>';
  echo '<div class="socialShareIcons2">';
  echo get_field('social_media_tekst', $mediaText) . '</br>';
...
if ($cat != 'Actueel' && $cat != 'Current') {
  ...
  echo '</a>';
    ...
    echo '<div class="h4_container">';
    echo '<h4 class="green underlineGreen">';
    echo 'Interested in this project?';
    echo '</h4>';
    echo '</div>';
    echo '</div>';
    echo '<p>Contact us throught this button</p>';
    echo '<a href="#contact" data-ps2id="true" class="ps2id">';
    echo '<button class="btn btn-green bezichtigings-btn-btn btn-primary">Contact</button>';
    echo '</a>';
    echo '</div>';
...
<div class="other_stories">
  ...
  <div class="h4_block">
    <div class="h4_container">
      <h4 class="underlineGreen">
        ...
        if ($cat == 'Actueel' || $cat == 'Current') {
          $mediaText = (ICL_LANGUAGE_CODE == 'nl') ? 1939 : 3295;
          printf(get_field('soortgelijke_projecten_titel', $mediaText));
        } else {
          if (ICL_LANGUAGE_CODE == 'nl') {
            printf("Soortgelijke projecten");
          } else {
            printf("Similar projects");
          }
        }
        ?>
      </h4>
  ...
  <?php
  $mediaText = (ICL_LANGUAGE_CODE == 'nl') ? 1939 : 3295;
  printf(get_field('soortgelijke_projecten_text', $mediaText)
  ); ?>
  ...
  <p><?php
     $mediaText = (ICL_LANGUAGE_CODE == 'nl') ? 1939 : 3295;
     printf(get_field('volledige_overzicht_text', $mediaText)); ?>
  </p>
  <a href="<?php
           $cat = get_the_category()[0]->name;
           if ($cat == 'Actueel') {
           ...
           if ($cat == 'Current') {
             printf(get_site_url() . '/actueel/?lang=en');
           } else {
             $source = (ICL_LANGUAGE_CODE == 'nl') ? '' : '/?lang=en';
             printf(get_site_url() . '/projecten' . $source);
           }
           ?>">
    <button class="btn btn-green btn-primary whiteBlue"><?php
                                                        $mediaText = (ICL_LANGUAGE_CODE == 'nl') ? 1939 : 3295;
                                                        printf(get_field('volledige-overzicht-button-text', $mediaText)); ?></button>
  </a>

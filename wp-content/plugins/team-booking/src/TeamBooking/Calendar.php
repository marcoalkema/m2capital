...
class Calendar
{
    ...
    public function getSlots(array $services, array $coworker_ids, $min_get_time = NULL, $max_get_time = NULL, $just_parse = FALSE, $timezone = NULL, $limit = 0)
    {
        ...
        // TODO: check if this could be enhanced
        if (function_exists('is_multisite') && is_multisite()) {
            $reservation_in_database = array_values($reservation_in_database);
            // multisite environment
            require_once ABSPATH . '/wp-admin/includes/plugin.php';
            global $wpdb;
            foreach ($wpdb->get_col("SELECT blog_id FROM $wpdb->blogs") as $blog_id) {
                $switched = switch_to_blog($blog_id);
                if ($switched && is_plugin_active(plugin_basename(TEAMBOOKING_FILE_PATH))) {
                    $reservation_in_database = array_merge($reservation_in_database, array_values(Database\Reservations::getByTime($min_get_time, $max_get_time)));
                }

                ...
                foreach ($items as $event_id => $event) {
                    ...
                    if (isset($this->service_matches[ $coworker_id ][ static::parseSummary($event->summary) ])) {
                        ...
                        if (substr(static::parseSummary($event->summary), -10) === ' container') {
                            ...
                            $slot->setReadOnly(\TeamBooking\Functions\tb_mb_strtolower($event->creator) !== \TeamBooking\Functions\tb_mb_strtolower(static::$settings->getCoworkerData($coworker_id)->getAuthAccount()));
